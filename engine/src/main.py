...
def keywordSearch(query: str, limit: int = 10, ids_to_exclude: list[int] | None = None) -> tuple[list[int], set[int]]:
    ...
    return (result[:limit], mandatory_ids)


def generateReadingList(user_prompt: str, limit: int = 10, ids_to_exclude: list[int] | None = None) -> list[int]:
    '''
    ...
    #         search.
    book_ids, ids_to_search = keywordSearch(user_prompt, limit_after_keyword, ids_to_exclude)
    ...
    book_ids = semanticSearch(user_prompt, limit_after_semantic, ids_to_search)
    for rank, id in enumerate(book_ids, start=1):
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Perform a semantic search for books.")
    ...
    # book_ids = semanticSearch(query)
    # for count, id in enumerate(book_ids, start=1):
    #     book = db_queries.fetchBookByID(id)
    #     if book:
    #         print(f"{count}: {book["title"]} by {",".join(book["authors"])}")
    #     else:
    #         print(f"{count}: Book id not found")

    # book_ids, _ = keywordSearch(query)
    # for count, id in enumerate(book_ids, start=1):
    #     book = db_queries.fetchBookByID(id)
    #     if book:
    #         print(f"{count}: {book["title"]} by {",".join(book["authors"])}")
    ...
    book_ids = generateReadingList(query)
    for count, id in enumerate(book_ids, start=1):
        book = db_queries.fetchBookByID(id)
        if book:
            print(f"{count}: {book["title"]} by {",".join(book["authors"])}")
